...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    public static readonly System.Version
        Version = new System.Version(4, 2, 15);
    ...
    public static readonly AstarDistribution
        Distribution = AstarDistribution.WebsiteDownload;
    ...
    public NavGraph[] graphs
    {
        get
        {
            if (data == null) data = new AstarData();
            return data.graphs;
        }
    }
...
#region MemoryStructures
    ...
    internal readonly HierarchicalGraph
    ...
    public static void FindAstarPath()
    {
        ...
        if (
            active != null &&
            (active.data.graphs == null || active.data.graphs.Length == 0)
                    ...
                    .GetNodes(node =>
                    {
                        if (
                            ...
                            Pathfinding
                                .Util
                                .GraphGizmoHelper
                                .InSearchTree(node, debugPathData, debugPathID)
                            )
                        )
                        {
                            ...
                            {
                                debugFloor =
                                    Mathf.Min(debugFloor, node.Penalty);
                            ...
                            else if (debugPathData != null)
                            {
                                ...
                                switch (debugMode)
                                {
                                    case GraphDebugMode.F:
                                        debugFloor =
                                            Mathf.Min(debugFloor, rnode.F);
                                        debugRoof =
                                            Mathf.Max(debugRoof, rnode.F);
                                        break;
                                    case GraphDebugMode.G:
                                        debugFloor =
                                            Mathf.Min(debugFloor, rnode.G);
                                        debugRoof =
                                            Mathf.Max(debugRoof, rnode.G);
                                        break;
                                    case GraphDebugMode.H:
                                        debugFloor =
                                            Mathf.Min(debugFloor, rnode.H);
                                        debugRoof =
                                            Mathf.Max(debugRoof, rnode.H);
                                        break;
                                }
                            }
                        }
    ...
    Pathfinding.Util.RetainedGizmos
        gizmos = new Pathfinding.Util.RetainedGizmos();
...
#region GraphUpdateMethods
    ...
    IEnumerator DelayedGraphUpdate()
    {
        ...
        yield return new WaitForSeconds(graphUpdateBatchingInterval -
                (Time.realtimeSinceStartup - lastGraphUpdate));
        QueueGraphUpdates();
    ...
    protected override void Awake()
    {
        ...
        {
            Debug
                .LogError("You should NOT have more than one AstarPath component in the scene at any time.\n" +
        ...
        if (
            scanOnStartup &&
            (!data.cacheStartup || data.file_cachedStartup == null)
        )
        {
            Scan();
        }
    ...
    {
        AstarProfiler
            .InitializeFastProfile(new string[14]
            {
                "Prepare", //0
                "Initialize", //1
                "CalculateStep", //2
    ...
    void OnDestroy()
    {
        ...
        if (logPathResults == PathLog.Heavy)
            Debug
                .Log("+++ AstarPath Component Destroyed - Cleaning Up Pathfinding Data +++");
        ...
        if (logPathResults == PathLog.Heavy) Debug.Log("Returning Paths");

        // Return all paths
        pathReturnQueue.ReturnPaths(false);

        if (logPathResults == PathLog.Heavy) Debug.Log("Destroying Graphs");

        // Clean up graph data
        data.OnDestroy();

        if (logPathResults == PathLog.Heavy) Debug.Log("Cleaning up variables");
...
#region ScanMethods
    ...
    public IEnumerable<Progress> ScanAsync(NavGraph[] graphsToScan = null)
    {
        ...
        for (int i = 0; i < graphsToScan.Length; i++)
        {
            ...
            float minp =
                Mathf.Lerp(0.1F, 0.8F, (float)(i) / (graphsToScan.Length));
            float maxp =
                Mathf
                    .Lerp(0.1F,
                    0.8F,
                    (float)(i + 0.95F) / (graphsToScan.Length));
            ...
            while (true)
            {
                ...
                yield return coroutine
                        .Current
                        .MapTo(minp, maxp, progressDescriptionPrefix);
            }
        }
        ...
        {
            Debug
                .Log("Scanning - Process took " +
                (lastScanTime * 1000).ToString("0") +
                " ms to complete");
        }
    }
    ...
    public static void BlockUntilCalculated(Path path)
    {
        ...
        {
            Debug
                .LogError("You are calling the BlockUntilCalculated function recursively (maybe from a path callback). Please don't do this.");
        }
    ...
    public static void StartPath(Path path, bool pushToFront = false)
    {
        ...
        {
            Debug
                .LogError("There is no AstarPath object in the scene or it has not been initialized yet");
            return;
        }

        if (path.PipelineState != PathState.Created)
        {
            throw new System.Exception("The path has an invalid state. Expected " +
                PathState.Created +
                " found " +
                path.PipelineState +
                "\n" +
    ...
    /// </summary>
    public NNInfo
    GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (graphs != null)
        {
            for (int i = 0; i < graphs.Length; i++)
            {
                ...
                float dist =
                    ((Vector3) nnInfo.clampedPosition - position).magnitude;
        ...
        )
        {
            ...
            NNInfoInternal nnInfo =
                graphs[nearestGraph].GetNearestForce(position, constraint);
            ...
            graph
                .GetNodes(node =>
                {
                    ...
                    Vector3 p =
                        lineOrigin +
                        (
                        Vector3.Dot(pos - lineOrigin, lineDirection) *
                        lineDirection
                        );
